...
class Score(object):
    ...
    def load_stop_words(self, stop_dict_path):
        stop_words = [w for w in open(stop_dict_path, 'r', encoding='UTF-8').readlines()]
    ...
    def load_sentiment_dict(self, dict_path):
        ...
        with open(dict_path, 'r', encoding='UTF-8') as f:
            ...
            for index, line in enumerate(f):
                ...
                polar = items[6]  # 极性,0为中性，1是褒义，2是贬义（-1），3兼有褒贬
                ...
                # 数值绝对值大小表示极性的强度 // 分红3类，极性：褒(+1)、中(0)、贬(-1)； 强度为权重值
                value = None
                if polar == 0:  # neutral
                    value = 0
                elif polar == 1:  # positive
                    value = intensity
                elif polar == 2:  # negtive，将2变为-1
                    value = -1 * intensity
                else:  # invalid
                    continue
                ...
                for item in self.score_class.items():
                    ...
                    for x in values:
                        if (sentiment == x):
                            class_value = key  # 若是values中包含，则获取key
    ...
    def get2score_position(self, words):
        ...
        for i in range(0, len(words)):
            if i in sen_locs:
                ...
                # senloc为情感词位置列表的序号,以前的sen_locs是情感词再分词后列表中的位置序号
                senloc += 1
                # score += W * float(sen_word[i])
                if (senloc == 0):  # 第一个情感词,前面是否有否认词，程度词
                    start = 0
                elif senloc < len(sen_locs):  # 和前面一个情感词之间，是否有否认词,程度词
                    # j为绝对位置
                ...
                for j in range(start, i):  # 词间的相对位置
                    ...
                    # 判断否认词和程度词的位置：1）否认词在前，程度词减半(加上正值)；不是很   2）否认词在后，程度加强（不变），很不是
    ...
    def get2score(self, text):
        word_list = self.findword(text)  ##查找文本中包含哪些正负情感词，而后分别分别累计它们的数值
        pos_score = 0
        pos_word = []
        neg_score = 0
        neg_word = []
    ...
    def getscore(self, text):
        ...
        not_w = 1
        not_word = []
        for notword in self.__class__.NOT_DICT:  # 否认词
            if notword in text:
                not_w = not_w * -1
...
if __name__ == '__main__':
    ...
    data = pd.read_csv(f).astype(str)       #.astype(str)解决jieba分词时读取gbk编码会错误的问题
